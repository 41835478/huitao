...
class Model
{
     ...
     */
    public $field = '*';
    public function __construct($table = null, $tablePrefix = null)
    {
        if(!self::$conn) {
            /**
             * [$conn 获取PDO实例]
             */
            self::$conn = new PDO(C('DB_DSN'),C('DB_USER'),C('DB_PWD'));
            /**
             * 指定编码集
             */
            $t = C('DB_CHARSET');
            empty($t) or self::$conn->exec('SET NAMES '.C('DB_CHARSET'));
        }
        /**
         * 设置表前缀
         */
        $t = C('DB_PREFIX');
        if(empty($tablePrefix) && !empty($t))
            $tablePrefix = C('DB_PREFIX');
        self::$table = $tablePrefix ? $tablePrefix.$table : $table;
    }
    /**
    ...
    {
        if($type === 'single') {
            return self::$conn->query($sql)->fetch(PDO::FETCH_ASSOC);
        }else{
