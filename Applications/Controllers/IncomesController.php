...
class IncomesController extends AppController
{
	...
	public function buySuccess($order_list=[])
	{
		...
		$order_info = M()->query("select distinct(o.order_id) , o.uid , o.cost ,o.deal_price , o.rating,u.sfuid from gw_order o join gw_uid u on o.uid = u.objectId where o.order_id in ({$list}) and o.status = 1 ",'all');
		$str = '';
		$str2 = '';
		$str3 = '';
		// D($order_info);die;
		...
		// $imei = $this->interface($order_uids);
		// if($imei) {
			// HeadacheController::queryActiv($imei);
		// }
	}
	...
	public function interface($uid_list = [])
	{
		if(!empty($uid_list))
		{
			...
			$uid = rtrim($uid,',');
			$imei_list = M()->query("select imei from gw_uid where objectId in ({$uid}) ",'all');
			foreach ($imei_list as $k => $v)
			{
				if(!empty($v['imei'])) $temp[] = $v['imei'];
			}
			return $temp;
		}
		return false;
