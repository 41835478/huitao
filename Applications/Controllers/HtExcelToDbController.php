...
class HtExcelToDbController extends Controller
{
    ...
    public function export(){
        ...
        $flag_goods=$this->addByExcel($mydate);
        if($flag_goods){
            $flag_coupon=$this->addcoupon($mydate);
            if($flag_coupon){
    ...
    public function addByExcel($mydate)
    {
        echo( "导入商品表开始...".date("Y-m-d H:i:s")."<br/>");
        $filePath=DIR.'/resource/goods_excel/goods'.$mydate.'.xls';
//        echo $filePath;
        $fmtdate=date("Y-m-d",strtotime($mydate));
        $data = self::format_excel2array($filePath,$fmtdate);
    ...
    //添加优惠券到数据库
    public function addcoupon($mydate)
    {
    ...
    function format_excel2array($filePath = '',$date='' ,$sheet = 0)
    {
