...
public function getData()
    ...
    function getItems($str){
        ...
        $m=preg_match('/http:\/\/[\w|.|\/|\?|\=]+/', $str, $match_link);
        if($m){
            $arr['link']=$match_link[0];
        }else{
            $arr['link']=-1;
        }
    ...
    function explain2($str){
        ...
        if(preg_match('/[0-9|\.]+元优惠券/', $str, $match_reduce)){
            ...
            $arr['reduce']=preg_split('/\>|\</',$match_reduce[0])[2];
        }else{
            //表示没有匹配到
            $arr['reduce']=-1;
        };
        ...
        //获取售价
        $f=preg_match('/[0-9|\.]+\<\/span\>目前/', $str, $match_price);
        if($f){
            $arr['price']=substr($match_price[0],0,-13);
        }else{
            $arr['price']=-1;
        }
        ...
        //获取剩余优惠券数量
        $b=preg_match('/优惠券剩余.+\>[0-9]+\<\/span\>张/', $str, $match_num_str);
        if($b){
            preg_match('/[0-9]+/',substr($match_num_str[0],40),$match_num); //;font-family:Arial,Helvetica,sans-serif;">9920</span>张
            $arr['num']=$match_num[0];
        }
        else{
            $arr['num']=-1;
        ...
        //获取已用优惠券数量
        $s=preg_match('/已领.+过期/', $str, $match_sita);
        if($s){
            preg_match('/[0-9]+/',substr($match_sita[0],40),$sita);
            //获取总共的优惠券数量
            $arr['sum']=$arr['num']+$sita[0];
        }else{
            $arr['sum']=-1;
        }

        //获取优惠券限额，limited
        $k=preg_match('/[0-9]+元可用/', $str, $match_limited);
        if($k){
            preg_match('/[0-9|\.]+/',$match_limited[0],$match_limited_str);
            $arr['limited']=$match_limited_str[0];
        }
        else{
            $arr['limited']=-1;
        }
        ...
        //获取销量  volume
        $aa=preg_match('/目前.+[0-9]+\<\/span\>/', $str, $match_volume);
        if($aa){
            $arr['volume']=preg_split('/\>|\</',$match_volume[0])[2];
        }else{
            $arr['volume']=-1;
        }
