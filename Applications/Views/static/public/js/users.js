$('document').ready(function () {
	...
	function showtable(data,pageno,psize,pnum){//要显示的数据，显示的页码，每页数据的条数，总页数，当前页码
		...
		for(var i=0;i<data.length;i++){
			var child="<tr><td>"
				...
				+checkstr(data[i]["nickname"])+"</td><td>"
				+checkstr(data[i]["objectId"])+"</td><td>"
				+checkstr(data[i]["price"])+"</td><td>"
				+checkstr(data[i]["sfuid"])+"</td><td style='width:150px;'>"
				+checkstr(data[i]["phone"])+"</td><td>"
				+checkstr(data[i]["alipay_name"])+"</td><td>"
				+checkstr(data[i]["alipay"])+"</td>"
	...
	function doget(postdata,currentno){
		var mainUrl=getUrl();
		var funUrl="/HtUserController/queryusers";
